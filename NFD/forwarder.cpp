...
Forwarder::onIncomingInterest(Face& inFace, const Interest& interest)
{
  ...
  if (faceUri[0] =='a')
  {
    switch(gotInterest[1])
    {
      ...
      case 'p':
      {
        ...
        //get Pit (or the inFace of current interest stored in PIT) and return
      ...
      case 'f':
      {
        ...
        std::string fTag = gotInterest.substr(fp+1);
  ...
  //uint64_t fp = interest.getName().toUri().find("/build");
...
/*
  if (fp != std::string::npos) 
  {
    ...
    if (faceUri[0] =='n') 
    {
      m_receiveNetFace = inFace.getId();
      std::cout << "receive net inface: " << m_receiveNetFace << std::endl;
    
      Name netDataName("ownNfd");
      auto netData = make_shared<Data>();
      netData->setName(netDataName);
      std::string net = boost::lexical_cast<std::string>(m_receiveNetFace);
      const uint8_t* netP = reinterpret_cast<const uint8_t*>(net.c_str());
      auto netBuffer = make_shared< ::ndn::Buffer>(netP, net.size());
      netData->setContent(netBuffer);
      inFace.sendData(cref(*netData));
     
    } 
    else if (faceUri[0] =='a')
    {
      char flagChar = interest.getName().toUri().at(1);
       
      if (flagChar == 'l') 
      {
        std::string buildTree = interest.getName().toUri();
        uint64_t l1 = buildTree.find("(");
        uint64_t l2 = buildTree.find(")");
        std::string reducerId = buildTree.substr(l1+1, l2-l1-1);
        std::cout << "reducer id: " << reducerId << std::endl;
    
        const fib::Entry& fibEntryToCurrentReducer = m_fib.findLongestPrefixMatch(reducerId);
        const fib::NextHopList& nextHopsToCurrentReducer = fibEntryToCurrentReducer.getNextHops();
        uint64_t fibFaceId = 0;
        for (const fib::NextHop& eachNextHop : nextHopsToCurrentReducer) 
        {
          fibFaceId = eachNextHop.getFace().getId();
          //std::cout << "fib next face is: " << fibFaceId << std::endl;
          std::cout << "face cost is: " << eachNextHop.getCost() << std::endl;
        }
      Name dataName(interest.getName());
      auto nfdData = make_shared<Data>();
      nfdData->setName(dataName);
      std::string r1 = boost::lexical_cast<std::string>(fibFaceId);
      std::string r2 = ",";
      std::string r3 = boost::lexical_cast<std::string>(m_receiveNetFace);
      std::string rawData = r1+r2+r3;
      const uint8_t* p = reinterpret_cast<const uint8_t*>(rawData.c_str());
      auto buffer = make_shared< ::ndn::Buffer>(p, rawData.size());
      nfdData->setContent(buffer);
      inFace.sendData(cref(*nfdData));

     //this->onOutgoingData(cref(*nfdData), inFace);
      }
    }
